using NUnit.Framework;
using UnityEngine;
using Narazaka.VRChat.Jnto.Editor.Phase2.Gate;
using Narazaka.VRChat.Jnto.Editor.Phase2.GpuPipeline;
using Narazaka.VRChat.Jnto.Editor.Phase2.Tiling;

public class BandingMetricTests
{
    [Test]
    public void SmoothGradient_VsQuantized_DetectsBanding()
    {
        var smooth = MakeGradient(128, quantize: 0);   // 連続グラデ
        var banded = MakeGradient(128, quantize: 8);   // 8 レベル量子化 → 階段化

        var grid = UvTileGrid.Create(128, 128);
        MarkAllCovered(grid);
        var r = FullR(grid);
        var calib = DegradationCalibration.Default();

        using (var ctxA = GpuTextureContext.FromTexture2D(smooth))
        using (var ctxB = GpuTextureContext.FromTexture2D(banded))
        {
            var metric = new BandingMetric();
            var scores = new float[grid.Tiles.Length];
            metric.Evaluate(ctxA.Original, ctxB.Original, grid, r, calib, scores);

            float maxScore = 0f;
            foreach (var s in scores) if (s > maxScore) maxScore = s;
            Assert.Greater(maxScore, 0.1f, "quantized gradient should produce banding score");
        }

        Object.DestroyImmediate(smooth);
        Object.DestroyImmediate(banded);
        Object.DestroyImmediate(calib);
    }

    [Test]
    public void Identical_NearZero()
    {
        // sRGB RT 化に伴う曲率アーティファクトを避けるため、
        // "identical" 検証はフラット (solid) テクスチャで行う。
        // gradient を orig=cand で渡すと sRGB curvature 由来の d2 が非ゼロになり
        // 全タイル評価 (bug A 修正後) で false-positive が立つ。
        var t = MakeSolid(128, new Color(0.5f, 0.5f, 0.5f, 1f));
        var grid = UvTileGrid.Create(128, 128);
        MarkAllCovered(grid);
        var r = FullR(grid);
        var calib = DegradationCalibration.Default();

        using (var ctx = GpuTextureContext.FromTexture2D(t))
        {
            var metric = new BandingMetric();
            var scores = new float[grid.Tiles.Length];
            metric.Evaluate(ctx.Original, ctx.Original, grid, r, calib, scores);

            foreach (var s in scores)
                Assert.Less(s, 0.05f);
        }

        Object.DestroyImmediate(t);
        Object.DestroyImmediate(calib);
    }

    [Test]
    public void NoCoverage_Zero()
    {
        var smooth = MakeGradient(128, quantize: 0);
        var banded = MakeGradient(128, quantize: 8);
        var grid = UvTileGrid.Create(128, 128);
        var r = new float[grid.Tiles.Length];
        var calib = DegradationCalibration.Default();

        using (var ctxA = GpuTextureContext.FromTexture2D(smooth))
        using (var ctxB = GpuTextureContext.FromTexture2D(banded))
        {
            var metric = new BandingMetric();
            var scores = new float[grid.Tiles.Length];
            metric.Evaluate(ctxA.Original, ctxB.Original, grid, r, calib, scores);
            foreach (var s in scores)
                Assert.AreEqual(0f, s, 0.001f);
        }

        Object.DestroyImmediate(smooth);
        Object.DestroyImmediate(banded);
        Object.DestroyImmediate(calib);
    }

    static void MarkAllCovered(UvTileGrid g)
    {
        for (int i = 0; i < g.Tiles.Length; i++)
            g.Tiles[i] = new TileStats { HasCoverage = true, Density = 100f, BoneWeight = 1f };
    }

    static float[] FullR(UvTileGrid g)
    {
        var r = new float[g.Tiles.Length];
        for (int i = 0; i < r.Length; i++) r[i] = g.TileSize;
        return r;
    }

    static Texture2D MakeSolid(int n, Color c)
    {
        var t = new Texture2D(n, n, TextureFormat.RGBA32, false);
        var px = new Color[n * n];
        for (int i = 0; i < px.Length; i++) px[i] = c;
        t.SetPixels(px);
        t.Apply();
        return t;
    }

    [Test]
    public void VerticalGradient_VsQuantized_DetectsBanding()
    {
        var smooth = MakeVerticalGradient(128, quantize: 0);
        var banded = MakeVerticalGradient(128, quantize: 8);

        var grid = UvTileGrid.Create(128, 128);
        MarkAllCovered(grid);
        var r = FullR(grid);
        var calib = DegradationCalibration.Default();

        using (var ctxA = GpuTextureContext.FromTexture2D(smooth))
        using (var ctxB = GpuTextureContext.FromTexture2D(banded))
        {
            var metric = new BandingMetric();
            var scores = new float[grid.Tiles.Length];
            metric.Evaluate(ctxA.Original, ctxB.Original, grid, r, calib, scores);

            float maxScore = 0f;
            foreach (var s in scores) if (s > maxScore) maxScore = s;
            Assert.Greater(maxScore, 0.1f,
                "vertically quantized gradient should produce banding score (y-direction detection)");
        }

        Object.DestroyImmediate(smooth);
        Object.DestroyImmediate(banded);
        Object.DestroyImmediate(calib);
    }

    static Texture2D MakeVerticalGradient(int n, int quantize)
    {
        var t = new Texture2D(n, n, TextureFormat.RGBA32, false);
        var px = new Color[n * n];
        for (int y = 0; y < n; y++)
        for (int x = 0; x < n; x++)
        {
            float v = y / (float)(n - 1);
            if (quantize > 0) v = Mathf.Round(v * (quantize - 1)) / (quantize - 1);
            px[y * n + x] = new Color(v, v, v, 1f);
        }
        t.SetPixels(px); t.Apply();
        return t;
    }

    [Test]
    public void NoisyGradient_VsQuantized_StillDetectsBanding()
    {
        var noisy = MakeNoisyGradient(128, noiseAmplitude: 0.012f);
        var banded = MakeGradient(128, quantize: 8);

        var grid = UvTileGrid.Create(128, 128);
        MarkAllCovered(grid);
        var r = FullR(grid);
        var calib = DegradationCalibration.Default();

        using (var ctxA = GpuTextureContext.FromTexture2D(noisy))
        using (var ctxB = GpuTextureContext.FromTexture2D(banded))
        {
            var metric = new BandingMetric();
            var scores = new float[grid.Tiles.Length];
            metric.Evaluate(ctxA.Original, ctxB.Original, grid, r, calib, scores);

            float maxScore = 0f;
            foreach (var s in scores) if (s > maxScore) maxScore = s;
            Assert.Greater(maxScore, 0.05f,
                "banding should be detected even with slightly noisy original gradient");
        }

        Object.DestroyImmediate(noisy);
        Object.DestroyImmediate(banded);
        Object.DestroyImmediate(calib);
    }

    static Texture2D MakeNoisyGradient(int n, float noiseAmplitude)
    {
        var t = new Texture2D(n, n, TextureFormat.RGBA32, false);
        var px = new Color[n * n];
        for (int y = 0; y < n; y++)
        for (int x = 0; x < n; x++)
        {
            float v = x / (float)(n - 1);
            int hash = (x * 73856093) ^ (y * 19349663);
            float noise = ((hash & 0xFFFF) / 65535f - 0.5f) * 2f * noiseAmplitude;
            v = Mathf.Clamp01(v + noise);
            px[y * n + x] = new Color(v, v, v, 1f);
        }
        t.SetPixels(px);
        t.Apply();
        return t;
    }

    static Texture2D MakeGradient(int n, int quantize)
    {
        var t = new Texture2D(n, n, TextureFormat.RGBA32, false);
        var px = new Color[n * n];
        for (int y = 0; y < n; y++)
        for (int x = 0; x < n; x++)
        {
            float v = x / (float)(n - 1);
            if (quantize > 0)
            {
                v = Mathf.Round(v * (quantize - 1)) / (quantize - 1);
            }
            px[y * n + x] = new Color(v, v, v, 1f);
        }
        t.SetPixels(px);
        t.Apply();
        return t;
    }
}
